...
acc = []
length = np.load("length_penguins.npy")
np.random.shuffle(length)
import random
for p in range(10):

    data=pd.read_csv('penguins_size.csv',header=None,sep=',')
    ...
    train_x = np.zeros((200, 4))
    train_y = np.zeros((200, 3))
    test_x = np.zeros((142, 4))
    test_y = np.zeros((142, 3))
    ...
    print(train_x.shape)
    kernel_value = generator.kernel_value(train_x[:, :], 2)
    ...
    #np.save(f"penguins/e/loss_iris{p+1}.npy", Lossclass)
    #np.save(f"penguins/fitness/loss_iris{p+1}.npy", fitness)
    #np.save(f"penguins/loss/loss_iris{p+1}.npy", Loss)
    ...
    kernel_value_test = generator.kernel_value(test_x[:, :], 2)

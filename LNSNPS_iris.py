...
for p in range(10):
    ...
    plt.xlim((0, maxgen+1))
    plt.ylim((0, 1.05))
    fitness_fig = plt.plot(Loss,'r')

    plt.plot(Loss,'b')

    #np.save(f"iris/e/loss_iris{p + 1}.npy", Lossclass)
    #np.save(f"iris/fitness/loss_iris{p + 1}.npy", fitness)
    #np.save(f"iris/loss/loss_iris{p + 1}.npy", Loss)
